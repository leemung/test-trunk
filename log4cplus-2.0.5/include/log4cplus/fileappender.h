...
namespace log4cplus
{
	...
	class LOG4CPLUS_EXPORT SizeAndTimeBasedRollingFileAppender : public FileAppenderBase {
	public:
		// Ctors
		SizeAndTimeBasedRollingFileAppender(const tstring& filename = LOG4CPLUS_TEXT(""),
			const tstring& filenamePattern = LOG4CPLUS_TEXT("%d.log"),
			long maxFileSize_= 10 * 1024 * 1024, 
			...
			bool createDirs = false,
			bool rollOnClose = true);
		SizeAndTimeBasedRollingFileAppender(const helpers::Properties& properties);

		// Dtor
		~SizeAndTimeBasedRollingFileAppender();

	protected:
		void append(const spi::InternalLoggingEvent& event);
		void open(std::ios_base::openmode mode);
		void close();
		void rolloverbytime(bool alreadyLocked = false);
		void rolloverbysize(bool alreadyLocked = false);
		void clean(helpers::Time time);
		helpers::Time::duration getRolloverPeriodDuration() const;
		helpers::Time calculateNextRolloverTime(const helpers::Time& t) const;
	...
	private:
		//LOG4CPLUS_PRIVATE void init();
		LOG4CPLUS_PRIVATE void init(long maxFileSize_, int maxBackupIndex_);
	};

	typedef helpers::SharedObjectPtr<SizeAndTimeBasedRollingFileAppender>
		SharedSizeAndTimeBasedRollingFileAppenderPtr;
